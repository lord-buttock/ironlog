...
## Priority 3 — Muscle Group Anatomy Diagrams (PNG replacement)

**Status:** Planned — after all exercise icons are complete  
**Context:** The current `MuscleDiagram` component is an inline SVG using geometric shapes (ellipses, paths) to approximate a front/back body figure. It renders correctly and highlights primary/secondary muscle groups in blue, but the visual quality is poor — the figure reads as a schematic rather than an illustration.

### Plan

Replace the SVG component with per-exercise PNG anatomy images generated by Codex, using the same AI illustration + Pillow cleanup pipeline used for exercise icons.

**Two options under consideration:**
1. **Per-exercise PNGs** — one anatomy image per exercise ID (e.g. `assets/muscles/goblet_squat.png`). Exact and always correct. High asset count (~40+ files).
2. **Per-muscle-group PNGs** — one image per primary muscle group (e.g. `assets/muscles/legs_front.png`, `assets/muscles/push.png`). Smaller asset set, reusable across exercises.

**Preferred approach:** Per-muscle-group is likely sufficient, since most exercises share the same highlight pattern within a group. Confirm with Phill before generating.

**Generation prompt direction:**
- Same approved illustration style as exercise icons (shaped head, subtle shading, professional fitness app aesthetic)
- Front and back body view showing highlighted muscle region(s) in blue (`#5b9df5`)
- Non-highlighted regions in pale neutral tone
- Transparent background, 108×160px (portrait to fit front+back in one image), or two images side-by-side at ~220×108px

**Implementation:**
- Add a `PNG_MUSCLE_DIAGRAM_IDS` Set (similar to `PNG_EXERCISE_ICON_IDS`)
- `MuscleDiagram` component checks the Set; if present, renders `<img src="assets/muscles/{key}.png" />`; falls back to inline SVG otherwise
- Keep inline SVG fallback intact until all PNGs are generated and verified

**Dependency:** Complete all exercise icons first (Workouts A, B, C + full PRESET_LIBRARY).
